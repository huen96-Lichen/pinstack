...
interface ModernRecordCardTextProps {
...
export function ModernRecordCardText({
  item,
  selected,
  onSelect,
  actions
}: ModernRecordCardTextProps): JSX.Element {
  const {
    hovered, setHovered,
  ...
  const [busy, setBusy] = useState<
    'copy' | 'copyLink' | 'open' | 'delete' | 'pin' | 'meta' | 'edit' | 'save' | 'favorite' | 'retry' | 'repair' | 'rename' | 'vaultkeeper' | null
  >(null);

  const run = async (
    type: 'copy' | 'copyLink' | 'open' | 'delete' | 'pin' | 'meta' | 'edit' | 'save' | 'favorite' | 'retry' | 'repair' | 'rename' | 'vaultkeeper',
    task: () => Promise<void>
  ) => {
    setBusy(type);
    try {
  ...
  return (
    <article
      ...
      style={cardGlowStyle}
      className={`motion-card glass-surface glass-l1 radius-l2 group relative break-inside-avoid border p-4 hover:shadow-[0_12px_28px_rgba(2,6,23,0.2)] ${
    ...
    >
      ...
      {isEditing ? (
        <textarea
          ...
          onClick={(event) => event.stopPropagation()}
          className="radius-control mt-2 min-h-[120px] w-full border border-cyan-300/70 bg-white/80 px-3 py-2 text-sm leading-6 text-black outline-none focus:border-cyan-400"
        />
      ) : (
        <p
