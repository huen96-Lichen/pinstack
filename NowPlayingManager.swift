...
//  PinStackNotch — 方案 C: MediaRemoteAdapter 子进程
//
//  通过 BoringNotch 的 MediaRemoteAdapter.framework + mediaremote-adapter.pl
//  以子进程 JSON Lines 流方式获取 Now Playing 信息。
//  控制命令（播放/暂停/上下曲/跳转）直接在 Swift 中调用系统 MediaRemote.framework。
//
...
private struct AdapterPayload: Codable {
    ...
    let artworkData: String?       // Base64 编码的封面
    let playing: Bool?
...
class NowPlayingManager: ObservableObject {
    ...
    init() {
        ...
        let foundFramework: String?

        // 优先从 SPM Bundle.module 查找
        #if SWIFT_PACKAGE
        foundScript = Bundle.module.url(forResource: "mediaremote-adapter", withExtension: "pl")?.path
        if let fwURL = Bundle.module.url(forResource: "MediaRemoteAdapter", withExtension: "framework") {
            foundFramework = fwURL.path
        } else {
            foundFramework = nil
        }
        #else
        ...
        adapterScriptPath = foundScript ?? (FileManager.default.fileExists(atPath: devScript) ? devScript : "")
        adapterFrameworkPath = foundFramework ?? (FileManager.default.fileExists(atPath: devFramework + "/Versions/A/MediaRemoteAdapter") ? devFramework : "")

        guard !adapterScriptPath.isEmpty, !adapterFrameworkPath.isEmpty else {
            NSLog("[NowPlaying] ❌ 找不到 MediaRemoteAdapter 资源")
            NSLog("[NowPlaying]   脚本: %@", adapterScriptPath.isEmpty ? "未找到" : adapterScriptPath)
            NSLog("[NowPlaying]   框架: %@", adapterFrameworkPath.isEmpty ? "未找到" : adapterFrameworkPath)
            sendCommandFunc = nil
            setElapsedTimeFunc = nil
            return
        }

        // 加载系统 MediaRemote.framework 用于控制命令
        var cmdFunc: (@convention(c) (Int, AnyObject?) -> Void)? = nil
        var elapsedFunc: (@convention(c) (Double) -> Void)? = nil

        if let mrBundle = CFBundleCreate(
            kCFAllocatorDefault,
            NSURL(fileURLWithPath: "/System/Library/PrivateFrameworks/MediaRemote.framework"))
        {
            ...
            NSLog("[NowPlaying] 系统 MediaRemote: SendCommand=%@ SetElapsedTime=%@",
                  cmdFunc != nil ? "✅" : "❌", elapsedFunc != nil ? "✅" : "❌")
        } else {
            NSLog("[NowPlaying] ⚠️ 无法加载系统 MediaRemote.framework（控制命令不可用）")
        }

        sendCommandFunc = cmdFunc
        setElapsedTimeFunc = elapsedFunc
    ...
    private func handleAdapterUpdate(_ update: AdapterUpdate) {
        ...
        if let elapsed = payload.elapsedTime {
            ...
            if payload.playing == false {
                ...
                newState.elapsedTime = state.elapsedTime + dt
            } else {
    ...
    func togglePlayPause() {
        sendCommandFunc?(2, nil)  // TogglePlayPause
    }

    func nextTrack() {
        sendCommandFunc?(4, nil)  // NextTrack
    }

    func previousTrack() {
        sendCommandFunc?(5, nil)  // PreviousTrack
    }

    func seek(to time: Double) {
        setElapsedTimeFunc?(time)
    }
    ...
    func readJSONLines<T: Decodable>(as type: T.Type, onLine: @escaping (T) -> Void) async {
        while !Task.isCancelled {
            do {
                ...
                NSLog("[NowPlaying] Pipe 读取错误: %@", error.localizedDescription)
                break
            }
        }
    }
